...
export class SurveyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.publicService.viewRegulator(this.req_id).subscribe(
      (res: any) => {
    ...
    this.isLoading = true;
    this.publicService.getSurveyType().subscribe(
      (res: any) => {
        this.isLoading = false;
        this.data = res;
  ...
  submit(regulator_id, survey_id): void {
    ...
    this.publicService.saveSurvey(data, regulator_id, survey_id).subscribe(
      (res: any) => {
        this.data = res;
        console.log(this.data);
      },
